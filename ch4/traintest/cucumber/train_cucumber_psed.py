...
from model.cucumber import CucumberModel
...
# 创建带日期和序号的结果文件夹
def create_result_folder(project_root, model_name, dataset_type='c'):
    """
    创建结果文件夹，命名格式: 日期-模型-c-序号
...
class MultiTaskLoss(nn.Module):
    def __init__(self):
        ...
        self.log_sigma2 = nn.Parameter(torch.tensor(0.0))  # arch 的 log(σ)

    def forward(self, loss_weight, loss_arch):
        sigma1 = torch.exp(self.log_sigma1)
        sigma2 = torch.exp(self.log_sigma2)
        total_loss = (1 / (2 * sigma1**2)) * loss_weight + (1 / (2 * sigma2**2)) * loss_arch
        total_loss += self.log_sigma1 + self.log_sigma2  # 正则化项
        return total_loss


class CucumberMaskedPseudoDataset(torch.utils.data.Dataset):
    """
    生成图像 + 伪标签（与 CucumberDataset 输出对齐：weight, curvature占位, arch, level）。

    默认: FruVeg26/data/cucumber/masked/cucumber_masked.csv
    CSV列: image, weight, arch, grade(=level)
    """
    ...
    def __init__(self, masked_dir: str, csv_path: str, transform=None):
        ...
        required = {"image", "weight", "arch", "level"}
    ...
    def _resolve_img_path(self, img_field: str) -> str:
        ...
        if os.path.exists(candidate):
            return candidate
        return os.path.join(self.masked_dir, str(img_field))
    ...
    def __getitem__(self, idx):
        ...
        arch = torch.tensor(float(row["arch"]), dtype=torch.float32)
        curvature = torch.tensor(0.0, dtype=torch.float32)
        ...
        return image, weight, curvature, arch, level
...
def _compute_composite_loss(
    ...
    if loss_type == "multi_task":
        loss_weight = criterion_weight_arch(weight_pred.squeeze(), weight)
        loss_arch = criterion_weight_arch(arch_pred.squeeze(), arch)
        loss_level = criterion_class(level_pred, level)
    elif loss_type == "huber":
        loss_weight = criterion_weight(weight_pred.squeeze(), weight)
        loss_arch = criterion_arch(arch_pred.squeeze(), arch)
        loss_level = criterion_class(level_pred, level)
    elif loss_type == "mse":
        loss_weight = criterion_weight(weight_pred.squeeze(), weight)
        loss_arch = criterion_arch(arch_pred.squeeze(), arch)
    ...
    if stage == 1:
        if loss_type == "multi_task":
            loss = multi_task_loss(loss_weight, loss_arch) + gamma * loss_level
        else:
            loss = loss_weight + loss_arch + loss_level
    elif stage == 2:
        loss = loss_level
    else:
        raise ValueError(f"unknown stage: {stage}")

    return loss, loss_weight, loss_arch, loss_level
...
def train_model(dataset, num_epochs, batch_size, lr, model_factory, loss_type="multi_task", 
    ...
    if masked_ratio and masked_ratio > 0:
        ...
        masked_ds = CucumberMaskedPseudoDataset(
    ...
    if stage == 2:
        statedict = "./save/1024-cucumber-once/best_model.pth"
        ...
        for param in model.curv_module.parameters():
            param.requires_grad = False
                        
        for param in model.weight_ffn.parameters():
            param.requires_grad = False

        for param in model.curv_ffn.parameters():
            param.requires_grad = False
    ...
    if loss_type == "multi_task":
        criterion_weight_arch = nn.HuberLoss()
        ...
        criterion_weight = nn.HuberLoss()
        criterion_arch = nn.HuberLoss()
        criterion_class = nn.CrossEntropyLoss()
        criterion_weight_arch = None
        multi_task_loss_orig = None
        multi_task_loss_masked = None
    elif loss_type == "mse":
        criterion_weight = nn.MSELoss()
        criterion_arch = nn.MSELoss()
        criterion_class = nn.CrossEntropyLoss()
        criterion_weight_arch = None
        multi_task_loss_orig = None
        multi_task_loss_masked = None
    ...
    for epoch in range(num_epochs):
        ...
        for step, data in enumerate(train_data_loader):
            ...
            loss_orig, loss_weight, loss_arch, loss_level = _compute_composite_loss(
                loss_type=loss_type,
                stage=stage,
                gamma=gamma,
                multi_task_loss=multi_task_loss_orig,
                criterion_weight_arch=criterion_weight_arch,
                criterion_weight=criterion_weight,
                criterion_arch=criterion_arch,
            ...
            if masked_iter is not None and omega and omega > 0:
                try:
                    m_inputs, m_weight, _, m_arch, m_level = next(masked_iter)
                ...
                m_arch = m_arch.to(device)
                m_level = m_level.to(device)

                mw_pred, ma_pred, ml_pred = model(m_inputs)
        ...
        with torch.no_grad():
            for step, data in enumerate(val_data_loader):
                ...
                all_true_archs.extend(arch.cpu().numpy())
                all_pred_archs.extend(arch_pred.squeeze().cpu().numpy())
                ...
                if stage == 1:
                    if loss_type == "multi_task":
                        ...
                        loss = multi_task_loss_orig(loss_weight, loss_arch) + gamma * loss_level
                    elif loss_type == "huber":
        ...
        if stage == 1:
            ...
            arch_r2 = r2_score(all_true_archs, all_pred_archs)
            arch_rmse = np.sqrt(mean_squared_error(all_true_archs, all_pred_archs))
        ...
        if stage == 2:
            if logger:
                ...
                logger.info(f'Loss Weight: {loss_weight.item()}, Loss Arch: {loss_arch.item()}, Loss Level: {loss_level.item()}')
            print(f'------ Epoch {epoch + 1}/{num_epochs} ------')
            print(f'Train Loss: {avg_train_loss}, Val Loss: {avg_val_loss}')
            print(f'Loss Weight: {loss_weight.item()}, Loss Arch: {loss_arch.item()}, Loss Level: {loss_level.item()}')
            ...
            if logger:
                ...
                logger.info(f'Loss Weight: {loss_weight.item():.4f}, Loss Arch: {loss_arch.item():.4f}, Loss Level: {loss_level.item():.4f}')
                if current_metrics:
                    logger.info(f'Weight R²: {current_metrics["weight_r2"]:.4f}, RMSE: {current_metrics["weight_rmse"]:.4f}')
                    logger.info(f'Arch R²: {current_metrics["arch_r2"]:.4f}, RMSE: {current_metrics["arch_rmse"]:.4f}')
            ...
            if current_metrics:
                ...
                print(f'Arch R²: {current_metrics["arch_r2"]:.4f}, RMSE: {current_metrics["arch_rmse"]:.4f}')
    ...
    if logger:
        ...
        if best_metrics:
            ...
            logger.info(f"Best metrics - Arch R²: {best_metrics['arch_r2']:.4f}, RMSE: {best_metrics['arch_rmse']:.4f}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='训练和测试Cucumber模型')
    ...
    parser.add_argument('--masked_dir', type=str, default=None,
                       help='生成(masked)图像目录，例如 FruVeg26/data/cucumber/masked')
    parser.add_argument('--masked_csv', type=str, default=None,
                       help='生成(masked)伪标签csv，例如 FruVeg26/data/cucumber/masked/cucumber_masked.csv')
    ...
    if args.masked_dir is None:
        args.masked_dir = os.path.join(project_root, "data", "cucumber", "masked")
    if args.masked_csv is None:
        args.masked_csv = os.path.join(args.masked_dir, "cucumber_masked.csv")
    ...
    result_folder, models_dir, logs_dir, figures_dir = create_result_folder(
        project_root, model_name, dataset_type='c'
    )
    ...
    # 设置日志
    log_file = os.path.join(logs_dir, 'cucumber_training.log')
    logging.basicConfig(
        level=logging.INFO,
    ...
    dataset = CucumberDataset(img_dir_train, mode="train", transform=transform)

    def model_factory():
        return CucumberModel(
            args.pretrained, 
            feature_fusion_type=args.feature_fusion_type, 
            fpn_ablation_type=args.fpn_ablation_type, 
            combine_type=args.combine_type
    ...
    logger.info(f"开始使用{args.n_runs}个不同种子进行训练，数据集: cucumber")
    logger.info(f"结果保存路径: {result_folder}")
    print(f"\n开始使用{args.n_runs}个不同种子进行训练，数据集: cucumber")
    ...
    if all_best_metrics and all_best_metrics[0] is not None:
        ...
        all_arch_r2 = [m['arch_r2'] for m in all_best_metrics if m]
        all_arch_rmse = [m['arch_rmse'] for m in all_best_metrics if m]
        ...
        mean_arch_r2 = np.mean(all_arch_r2)
        std_arch_r2 = np.std(all_arch_r2, ddof=1)
        mean_arch_rmse = np.mean(all_arch_rmse)
        std_arch_rmse = np.std(all_arch_rmse, ddof=1)
        ...
        all_weight_r2 = all_weight_rmse = all_arch_r2 = all_arch_rmse = None
        ...
        mean_arch_r2 = std_arch_r2 = mean_arch_rmse = std_arch_rmse = None
    ...
    if all_best_metrics and all_best_metrics[0] is not None:
        ...
        print(f"  Arch R²: {mean_arch_r2:.4f} ± {std_arch_r2:.4f}")
        print(f"  Arch RMSE: {mean_arch_rmse:.4f} ± {std_arch_rmse:.4f}")
    ...
    if all_best_metrics and all_best_metrics[0] is not None:
        ...
        logger.info(f"平均 Arch R²: {mean_arch_r2:.4f} ± {std_arch_r2:.4f}")
    ...
    plt.title(f'Cucumber模型 - 验证准确率 ({args.n_runs}次运行)', fontsize=14)
    plt.xlabel('运行次数')
    plt.ylabel('验证准确率')
    ...
    # 保存最终结果到文件
    results_summary_path = os.path.join(result_folder, 'cucumber_results_summary.txt')
    with open(results_summary_path, 'w', encoding='utf-8') as f:
        f.write(f"{'='*80}\n")
        f.write(f"Cucumber模型 - 多次运行结果摘要\n")
        f.write(f"{'='*80}\n\n")
        f.write(f"实验配置:\n")
        f.write(f"  数据集: cucumber\n")
        ...
        if all_best_metrics and all_best_metrics[0] is not None:
            ...
            f.write(f"  Arch R²: {mean_arch_r2:.4f} ± {std_arch_r2:.4f}\n")
            f.write(f"  Arch RMSE: {mean_arch_rmse:.4f} ± {std_arch_rmse:.4f}\n")
            f.write(f"\n分类任务:\n")
        ...
        for i in range(args.n_runs):
            ...
            if all_best_metrics[i] is not None:
                ...
                f.write(f"  Arch R²: {m['arch_r2']:.4f}, RMSE: {m['arch_rmse']:.4f}\n")
