...
# -*- coding: utf-8 -*-
"""
Banana模型Wilcoxon signed-rank显著性分析

对以下模型进行两两比较：
1. BananaModel (验证集准确率)
2. CIDIS-Multi (测试集准确率)
3. FruitVision-Multi-Simple-b-1 (测试集准确率，冻结骨干)
4. FruitVision-Multi-Simple-b-2 (测试集准确率，非冻结骨干)
"""
...
# 定义各模型的准确率数据（10次运行）
model_data = {
    'BananaModel': np.array([0.8462, 0.9615, 0.9231, 0.9615, 0.8462, 0.9615, 0.8846, 0.8462, 0.9231, 0.8462]),
    'CIDIS-Multi': np.array([0.7333, 0.8667, 0.8667, 0.8667, 0.8000, 0.7667, 0.8667, 0.8333, 0.7667, 0.9000]),
    'FruitVision-Multi-冻结': np.array([0.7667, 0.7333, 0.7333, 0.8333, 0.8667, 0.6667, 0.7333, 0.6667, 0.8667, 0.6667]),
    'FruitVision-Multi-非冻结': np.array([0.7667, 0.7333, 0.9000, 0.5667, 0.7667, 0.6667, 0.7333, 0.6333, 0.7667, 0.2333]),
}
...
def main():
    """主函数"""
    alpha = 0.05
    output_dir = 'wilcoxon_banana_analysis'
    os.makedirs(output_dir, exist_ok=True)
    
    print("="*80)
    print("Banana模型Wilcoxon Signed-Rank显著性分析")
    print("="*80)
    print(f"\n显著性水平: α = {alpha}")
    print(f"\n各模型基本统计:")
    print("-"*80)
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("Banana模型Wilcoxon Signed-Rank显著性分析报告\n")
        f.write("="*80 + "\n\n")
