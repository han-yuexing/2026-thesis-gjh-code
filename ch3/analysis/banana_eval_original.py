...
def create_output_dir(base_dir: Path, model_tag: str = "BananaEvalOriginal-b") -> Path:
...
def build_report_content(
    ...
    lines: List[str] = []
    lines.append("=" * 80)
    lines.append("Banana模型 - 原始测试集评估摘要")
    lines.append("=" * 80)
    lines.append("")
    lines.append("评估配置:")
    lines.append(f"  数据集: {dataset_dir}/test ({dataset_size}张)")
    lines.append("  数据加载: BananaDataset (test.csv, Resize 224, ToTensor)")
    ...
    lines.append("")
    lines.append("要点:")
    lines.append("  - 重量与成熟度回归在小样本测试集上保持稳定。")
...
def main():
    parser = argparse.ArgumentParser(description="在原始香蕉测试集上评估多个模型并汇总结果")
    ...
    test_dataset = BananaDataset(image_dir=str(banana_dir / "test"), mode="test", transform=transform)

    best_models = find_best_models(model_root)
    output_root = create_output_dir(Path(args.output_base), model_tag="BananaEvalOriginal-b")
    ...
    metrics_csv = output_root / "banana_original_eval_metrics.csv"
    df.to_csv(metrics_csv, index=False)

    summary = df.describe().transpose()
    summary_csv = output_root / "banana_original_eval_summary.csv"
    summary.to_csv(summary_csv)

    summary_txt = output_root / "banana_original_eval_summary.txt"
    with open(summary_txt, "w", encoding="utf-8") as f:
        f.write("Banana Original Test Evaluation Summary\n")
        f.write("=" * 60 + "\n\n")
    ...
    report_txt = output_root / "banana_original_eval_report.txt"
