...
def extract_features_from_batch(batch, dataset_type):
    ...
    参数:
        batch: 数据加载器返回的batch
        dataset_type: 'banana' 或 'cucumber'
    
    返回:
        images, weight, shape_features, labels
    """
    if dataset_type == 'cucumber':
        ...
        images = batch[0]
        weight = batch[1]
        arch = batch[3]  # 只用arch，不用curvature
        labels = batch[4]
        shape_features = arch.unsqueeze(1)  # (batch_size, 1)
...
def train_model(dataset_type, transform, batch_size=16, epochs=50, lr=0.001, seed=42, 
    ...
    参数:
        use_simple_model: 是否使用简化版模型 (直接拼接特征，不使用编码器)
    """
    # 设置当前运行的随机种子
    set_seed(seed)
    ...
    # 初始化多模态模型
    if use_simple_model:
        model = CIDIS_Model_Multimodal_Simple(num_classes=len(class_names), 
                                              num_shape_features=num_shape_features).to(device)
    else:
        model = CIDIS_Model_Multimodal(num_classes=len(class_names), 
                                       num_shape_features=num_shape_features).to(device)
    ...
    if save_dirs and 'models' in save_dirs:
        best_model_path = os.path.join(save_dirs['models'], 
                                      f"best_cidis_multimodal_{model_suffix}_{dataset_type}_seed{seed}_run{run_id}.pth")
    else:
        best_model_path = f"best_cidis_multimodal_{model_suffix}_{dataset_type}_seed{seed}_run{run_id}.pth"
    ...
    if save_dirs and 'figures' in save_dirs:
        curves_path = os.path.join(save_dirs['figures'], 
                                  f'cidis_multimodal_{model_suffix}_run{run_id}_seed{seed}_curves_{dataset_type}.png')
    else:
        curves_path = f'cidis_multimodal_{model_suffix}_run{run_id}_seed{seed}_curves_{dataset_type}.png'
...
def test_model(dataset_type, model_path, transform, batch_size=16, class_names=None, 
               num_shape_features=None, save_dirs=None, device=None, logger=None, use_simple_model=False):
    """测试多模态模型"""
    if device is None:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    plt.title('Confusion Matrix - Multimodal CIDIS')
    plt.colorbar()
...
if __name__ == "__main__":
    ...
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description='CIDIS多模态模型训练和测试')
    ...
    # 创建结果文件夹
    model_name = "CIDIS-Multi" + ("-Simple" if args.simple else "")
    ...
    # 设置日志
    log_file = os.path.join(logs_dir, 'cidis_multimodal_training.log')
    logging.basicConfig(
        level=logging.INFO,
    ...
    logger.info(f"{'='*80}")
    logger.info(f"多模态CIDIS模型 - 最终测试结果摘要")
    logger.info(f"{'='*80}")
    ...
    plt.tight_layout()
    results_path = os.path.join(figures_dir, f'cidis_multimodal_multi_run_results_{args.dataset}.png')
    plt.savefig(results_path, dpi=300, bbox_inches='tight')
    plt.close()
    logger.info(f"多次运行结果对比图已保存: {results_path}")
    
    # 保存最终结果到文件
    results_summary_path = os.path.join(result_folder, f'cidis_multimodal_results_summary_{args.dataset}.txt')
    with open(results_summary_path, 'w', encoding='utf-8') as f:
        f.write(f"{'='*80}\n")
        f.write(f"多模态CIDIS模型 - {args.dataset.upper()}数据集 - 多次运行结果摘要\n")
        f.write(f"{'='*80}\n\n")
        f.write(f"实验配置:\n")
