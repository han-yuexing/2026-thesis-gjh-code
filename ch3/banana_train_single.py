    ...
    for epoch in range(num_epochs):
        ...
        for step, data in enumerate(train_data_loader):
            ...
            if switch == 1:
                ...
                loss = criterion(output.squeeze(), ripeness)
                true_train_values.extend(ripeness.cpu().numpy())
                pred_train_values.extend(output.squeeze().detach().cpu().numpy())  # 使用 detach
            elif switch == 3:
        ...
        with torch.no_grad():
            for step, data in enumerate(val_data_loader):
                ...
                if switch == 1:
                    ...
                    loss = criterion(output.squeeze(), ripeness)
                    true_val_values.extend(ripeness.cpu().numpy())
                    pred_val_values.extend(output.squeeze().detach().cpu().numpy())  # 使用 detach
                elif switch == 3:
                    loss = criterion(output, level)
                    _, predicted = torch.max(output, 1)
    ...
    with torch.no_grad():
        for step, data in enumerate(test_data_loader):
            ...
            if switch == 1:
                ...
                true_values.extend(ripeness.cpu().numpy())
                pred_values.extend(output.squeeze().detach().cpu().numpy())  # 使用 detach
            elif switch == 3:
...
if __name__ == "__main__":
    ...
    model_name = 'vgg16'
    img_dir = "./data/banana/train"
    test_img_dir = "./data/banana/test"
    ...
    dataset = BananaDataset(img_dir, mode="train", transform=transform)
    test_dataset = BananaDataset(test_img_dir, mode="test", transform=transform)
    ...
    # 第二次训练：预测 ripeness
    save_ripeness = f"./save/{model_name}/simple_train_ripeness/"
    os.makedirs(save_ripeness, exist_ok=True)
    logging.basicConfig(filename=save_ripeness+'train.log', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
    logging.info(f"Training for ripeness prediction")
    dataset = BananaDataset(img_dir, mode="train", transform=transform)
    test_dataset = BananaDataset(test_img_dir, mode="test", transform=transform)
    model_ripeness = train_and_validate(dataset, num_epochs, batch_size, lr, save_ripeness, modelpath, switch=2)
    test_model(test_dataset, model_ripeness, switch=2, save=save_ripeness)
