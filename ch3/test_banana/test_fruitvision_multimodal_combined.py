...
sys.path.append(os.path.join(project_root, 'FruitVision-multimodal'))

# 导入自定义模块
from data.dataset import BananaCombinedDataset
from fruit_vision_multimodal import FruitVision_Multimodal_Simple
...
def test_model(model_path, test_dataset, batch_size=16, pretrained=True, freeze_backbone=True, device=None, logger=None):
    """
    测试单个FruitVision多模态模型
    """
    if device is None:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='测试FruitVision多模态模型')
    parser.add_argument('--result_folder', type=str, required=True, 
                       help='结果文件夹路径，例如: results/20251029-FruitVision-Multi-Simple-b-1')
    ...
    model_name = f"FruitVision-Multi-Test-{freeze_str}"
    ...
    # 定义种子和对应的模型文件
    model_files = [
        'best_fruitvision_multimodal_simple_banana_seed24_run1.pth',
        'best_fruitvision_multimodal_simple_banana_seed42_run2.pth',
        'best_fruitvision_multimodal_simple_banana_seed2024_run3.pth',
        'best_fruitvision_multimodal_simple_banana_seed2025_run4.pth',
        'best_fruitvision_multimodal_simple_banana_seed3407_run5.pth',
        'best_fruitvision_multimodal_simple_banana_seed1234_run6.pth',
        'best_fruitvision_multimodal_simple_banana_seed5678_run7.pth',
        'best_fruitvision_multimodal_simple_banana_seed9999_run8.pth',
        'best_fruitvision_multimodal_simple_banana_seed7777_run9.pth',
        'best_fruitvision_multimodal_simple_banana_seed8888_run10.pth'
    ]
    ...
    # 保存结果到文件
    results_summary_path = os.path.join(new_result_folder, f'fruitvision_multimodal_results_summary_banana_combined_{freeze_str}.txt')
    with open(results_summary_path, 'w', encoding='utf-8') as f:
        f.write(f"{'='*80}\n")
        f.write(f"FruitVision多模态模型 - BANANA数据集（合并测试集） - 多次运行结果摘要\n")
        f.write(f"{'='*80}\n\n")
        f.write(f"实验配置:\n")
