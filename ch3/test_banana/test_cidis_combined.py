...
from CIDIS.CIDIS_model import CIDIS_Model
...
def test_model(model_path, test_dataset, batch_size=20, device=None, logger=None):
    """
    测试单个模型
    """
    if device is None:
    ...
    # 初始化模型
    model = CIDIS_Model(num_classes=4).to(device)
...
def main():
    ...
    # 加载配置文件
    config_path = os.path.join(project_root, 'results', '20251028-CIDIS-b-1', 'config.yaml')
    ...
    # 模型目录
    models_dir = os.path.join(project_root, 'results', '20251028-CIDIS-b-1', 'models')
    ...
    # 创建新的结果文件夹
    result_folder, logs_dir = create_result_folder(project_root, 'CIDIS-Test', dataset_type='b')
    ...
    with open(config_save_path, 'w', encoding='utf-8') as f:
        yaml.dump({
            ...
            'model_source': 'results/20251028-CIDIS-b-1/models'
    ...
    # 定义种子和对应的模型文件
    model_files = [
        'best_cidis_banana_seed24_run1.pth',
        'best_cidis_banana_seed42_run2.pth',
        'best_cidis_banana_seed2024_run3.pth',
        'best_cidis_banana_seed2025_run4.pth',
        'best_cidis_banana_seed3407_run5.pth',
        'best_cidis_banana_seed1234_run6.pth',
        'best_cidis_banana_seed5678_run7.pth',
        'best_cidis_banana_seed9999_run8.pth',
        'best_cidis_banana_seed7777_run9.pth',
        'best_cidis_banana_seed8888_run10.pth'
    ]
    ...
    # 保存结果到文件
    results_summary_path = os.path.join(result_folder, 'cidis_results_summary_banana_combined.txt')
    with open(results_summary_path, 'w', encoding='utf-8') as f:
        f.write(f"{'='*80}\n")
        f.write(f"CIDIS模型 - BANANA数据集（合并测试集） - 多次运行结果摘要\n")
        f.write(f"{'='*80}\n\n")
        f.write(f"实验配置:\n")
