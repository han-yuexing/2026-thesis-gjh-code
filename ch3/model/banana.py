...
__all__ = ["WeightModule", "RipeModule", "BananaModel"]
...
class RipeModule(nn.Module):
    def __init__(self, modelpath="resnet50.pth"):
        super(RipeModule, self).__init__()
        model_name = os.path.splitext(os.path.basename(modelpath))[0]
...
class BananaModel(nn.Module):
    def __init__(self, modelpath="resnet50.pth", feature_fusion_type="cross_attention", fpn_ablation_type="ripe_only", combine_type="lka"):
        ...
        self.ripe_module = RipeModule(modelpath)
        ...
        # 获取backbone的通道数
        backbone_channels = self.ripe_module.out_channels
    ...
    def forward(self, x):
        ...
        ripe_features = self.ripe_module(x)

        # 根据模型类型选择不同的特征层
        if self.model_name.startswith("vgg"):
            # VGG模型：使用所有4层特征
            weight_selected = weight_features
            ripe_selected = ripe_features
        else:
            # ResNet模型：选择后4层特征（跳过第一层）
            weight_selected = weight_features[-4:]
            ripe_selected = ripe_features[-4:]
        ...
        if self.fpn_ablation_type == "none":
            ...
            # 都有FPN：weight和ripe都使用FPN
            weight_fpn_features = self.fpn(weight_selected)
            ripe_fpn_features = self.fpn(ripe_selected)
...
if __name__ == "__main__":
    ...
    ripe_module = RipeModule(modelpath="model/resnet50.pth")
    features = ripe_module(input_tensor)
    for i, feature in enumerate(features):
        print(f"Feature {i+1} Shape: {feature.shape}")
