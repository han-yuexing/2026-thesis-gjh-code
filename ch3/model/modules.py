...
__all__ = ["LR_ASPP", "FPN", "CrossAttention", "LKA_Attention"]


class LR_ASPP(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(LR_ASPP, self).__init__()

        # aspp 1
        self.conv1_1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)

        # aspp 2
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.conv1_2 = nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        aspp1 = self.conv1_1(x)
        aspp1 = self.bn1(aspp1)
        aspp1 = self.relu(aspp1)

        aspp2 = self.avgpool(x)
        aspp2 = self.conv1_2(aspp2)
        aspp2 = self.sigmoid(aspp2)
...
if __name__ == "__main__":
    ...
    # Test LR_ASPP module
    print("Testing LR_ASPP Module...")
    lr_aspp = LR_ASPP(in_channels=64, out_channels=128)
    lr_aspp_output = lr_aspp(input_tensor_1)
    print(f"LR_ASPP Output Shape: {lr_aspp_output.shape}")  # Should be [1, 128, 32, 32]
