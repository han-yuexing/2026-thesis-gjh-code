...
__all__ = ["SharedBackbone", "CucumberModel"]
...
class CucumberModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.model_name.startswith("vgg"):
            ...
            curv_selected = shared_features
        else:
            # ResNet模型：选择后4层特征（跳过第一层）
            weight_selected = shared_features[-4:]
            curv_selected = shared_features[-4:]
        ...
        if self.fpn_ablation_type == "none":
            ...
            # 都有FPN：weight和curv都使用各自的FPN
            weight_fpn_features = self.weight_fpn(weight_selected)
            curv_fpn_features = self.curv_fpn(curv_selected)
...
if __name__ == "__main__":
    ...
    print("Testing CucumberModel with ResNet50 and Shared Backbone...")
    cucumber_model = CucumberModel(modelpath="model-SharedBackbone/pretrained/resnet50.pth")
    weight, curv, logits = cucumber_model(input_tensor)
    print(f"Weight Shape: {weight.shape}")
    print(f"Curv Shape: {curv.shape}")
    print(f"Logits Shape: {logits.shape}")
    print("CucumberModel Test Completed!")