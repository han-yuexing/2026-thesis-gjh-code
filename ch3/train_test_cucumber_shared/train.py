...
def train_and_cross_validate(dataset, num_epochs, batch_size, lr, model_factory, loss_type="multi_task",
                            n_folds=5, seed=3407, save_dirs=None, device=None, logger=None, stage=1, gamma=0.1):
    """
    训练和交叉验证函数
    loss_type: 损失函数类型
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(dataset)):
        ...
        if stage == 2:
            statedict = "./save/0624-cu-resnet34-train1-1/1/best_epoch98.pth"
            ...
            # 冻结两个FPN（如果存在）
            if hasattr(model, 'weight_fpn'):
                for param in model.weight_fpn.parameters():
                    param.requires_grad = False
                    
            if hasattr(model, 'curv_fpn'):
                for param in model.curv_fpn.parameters():
                    param.requires_grad = False
        ...
        if loss_type == "multi_task":
            criterion_weight_arch = nn.HuberLoss()  # 用于重量和成熟度的损失
            ...
            criterion_arch = nn.HuberLoss()  # 用于成熟度的损失
            criterion_class = nn.CrossEntropyLoss()  # 用于分类的损失
        elif loss_type == "mse":
            criterion_weight = nn.MSELoss()  # 用于重量的损失
            criterion_arch = nn.MSELoss()  # 用于成熟度的损失
            criterion_class = nn.CrossEntropyLoss()  # 用于分类的损失
        ...
        for epoch in range(num_epochs):
            ...
            for step, data in enumerate(train_data_loader):
                ...
                if loss_type == "multi_task":
                    ...
                    loss_arch = criterion_weight_arch(ripeness_pred.squeeze(), arch)
                    loss_level = criterion_class(level_pred, level)
                elif loss_type == "huber":
                    loss_weight = criterion_weight(weight_pred.squeeze(), weight)
                    loss_arch = criterion_arch(ripeness_pred.squeeze(), arch)
                    loss_level = criterion_class(level_pred, level)
                elif loss_type == "mse":
                    loss_weight = criterion_weight(weight_pred.squeeze(), weight)
                    loss_arch = criterion_arch(ripeness_pred.squeeze(), arch)
            ...
            if stage == 2:
                if logger:
                    ...
                    logger.info(f'Loss Weight: {loss_weight.item():.4f}, Loss Curvature: {loss_arch.item():.4f}, Loss Level: {loss_level.item():.4f}')
                print(f'Fold {fold + 1}, Epoch {epoch + 1}/{num_epochs}')
                print(f'Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}')
                print(f'Loss Weight: {loss_weight.item():.4f}, Loss Curvature: {loss_arch.item():.4f}, Loss Level: {loss_level.item():.4f}')
                ...
                print(f'Loss Weight: {loss_weight.item():.4f}, Loss Arch: {loss_arch.item():.4f}, Loss Level: {loss_level.item():.4f}')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='训练Cucumber模型（K-Fold交叉验证）')
    ...
    # 设置日志
    log_file = os.path.join(logs_dir, 'cucumber_kfold_training.log')
    logging.basicConfig(
        level=logging.INFO,
