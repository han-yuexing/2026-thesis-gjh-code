...
# 将 model-SharedBackbone 目录添加到 sys.path，以便 banana.py 能找到 modules.py
if model_shared_backbone_dir not in sys.path:
    sys.path.insert(0, model_shared_backbone_dir)
model_path = os.path.join(model_shared_backbone_dir, 'banana.py')
spec = importlib.util.spec_from_file_location("banana_shared", model_path)
banana_shared = importlib.util.module_from_spec(spec)
spec.loader.exec_module(banana_shared)
BananaModel = banana_shared.BananaModel
...
def test_model(test_dataset, model_path, pretrainpath, feature_fusion_type="cross_attention", 
    ...
    model = BananaModel(pretrainpath, feature_fusion_type=feature_fusion_type, fpn_ablation_type=fpn_ablation_type, combine_type=combine_type).to(device)
