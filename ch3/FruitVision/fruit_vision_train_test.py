...
from fruit_vision import FruitVision
...
class CLAHEOtsuTransform:
    ...
    def __call__(self, img):
        ...
        # 应用Otsu掩码到原图
        otsu_mask_3d = cv2.cvtColor(otsu_mask, cv2.COLOR_GRAY2BGR)
        img_processed = cv2.bitwise_and(img_clahe, otsu_mask_3d)
...
# 数据预处理 - 按照原文章要求
clahe_otsu_transform = CLAHEOtsuTransform(target_size=(224, 224))
transform = transforms.Compose([
    clahe_otsu_transform,
    transforms.ToTensor(),
...
def extract_labels_from_batch(batch):
    ...
    if isinstance(batch, (list, tuple)):
        ...
        # 对于 CucumberDataset: (image, weight, curvature, arch, level)
...
def train_model(dataset_type, transform, batch_size=16, epochs=100, lr=0.001, freeze_backbone=True, seed=42, run_id=1, save_dirs=None, device=None, logger=None):
    """训练 FruitVision 模型 - 使用固定的训练集和验证集
    ...
    if logger:
        logger.info(f"Starting training FruitVision on {dataset_type} dataset for {epochs} epochs...")
        logger.info(f"Freeze backbone: {freeze_backbone}")
        logger.info(f"Input size: 224x224 with CLAHE + Otsu preprocessing")
    ...
    if save_dirs and 'models' in save_dirs:
        best_model_path = os.path.join(save_dirs['models'], f"best_fruit_vision_{dataset_type}_seed{seed}_run{run_id}.pth")
    else:
        best_model_path = f"best_fruit_vision_{dataset_type}_seed{seed}_run{run_id}.pth"
    ...
    if save_dirs and 'figures' in save_dirs:
        training_curves_path = os.path.join(save_dirs['figures'], f'fruit_vision_run{run_id}_seed{seed}_curves_{dataset_type}.png')
    else:
        training_curves_path = f'fruit_vision_run{run_id}_seed{seed}_curves_{dataset_type}.png'
...
def test_model(dataset_type, model_path, transform, batch_size=16, class_names=None, save_dirs=None, device=None, logger=None):
    ...
    # 创建模型并加载权重
    model = FruitVision(num_classes=len(class_names), pretrained=False).to(device)
    ...
    # 计算specificity
    specificity_scores = calculate_specificity(final_labels, final_predictions, class_names)
    ...
    if logger:
        logger.info("Detailed Metrics per Class:")
    for i, class_name in enumerate(class_names):
        class_metrics = f"{class_name}: Precision={precision[i]:.4f}, Recall={recall[i]:.4f}, F1={f1[i]:.4f}, Specificity={specificity_scores[class_name]:.4f}"
        print(class_metrics)
        if logger:
    ...
    print(f"\nMacro Average: Precision={macro_precision:.4f}, Recall={macro_recall:.4f}, F1={macro_f1:.4f}, Specificity={avg_specificity:.4f}")
    print(f"Weighted Average: Precision={weighted_precision:.4f}, Recall={weighted_recall:.4f}, F1={weighted_f1:.4f}")
    
    if logger:
        logger.info(f"Macro Average: Precision={macro_precision:.4f}, Recall={macro_recall:.4f}, F1={macro_f1:.4f}, Specificity={avg_specificity:.4f}")
        logger.info(f"Weighted Average: Precision={weighted_precision:.4f}, Recall={weighted_recall:.4f}, F1={weighted_f1:.4f}")
    ...
    test_results_dict = {
        ...
        'recall': recall,
        'f1_score': f1,
        'specificity': list(specificity_scores.values()),
        'support': support,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='训练和测试FruitVision模型')
    ...
    parser.add_argument('--freeze_backbone', action='store_true', 
                       default=config['training']['freeze_backbone'],
                       help='冻结主干网络参数')
    ...
    if args.test_only:
        ...
        test_model(args.dataset, args.test_only, transform, args.batch_size)
    else:
        # 确保种子数量与运行次数匹配
        ...
        # 设置日志
        log_file = os.path.join(logs_dir, 'fruit_vision_training.log')
        logging.basicConfig(
            level=logging.INFO,
        ...
        freeze_status = "冻结" if args.freeze_backbone else "非冻结"
        logger.info(f"开始使用{args.n_runs}个不同种子进行训练，数据集: {args.dataset}，骨干网络: {freeze_status}")
        logger.info(f"结果保存路径: {result_folder}")
        print(f"\n开始使用{args.n_runs}个不同种子进行训练，数据集: {args.dataset}，骨干网络: {freeze_status}")
        ...
        macro_specificities = [result['macro_specificity'] for result in test_results_list]
        ...
        logger.info(f"宏平均 - Specificity: {mean_macro_specificity:.4f} ± {std_macro_specificity:.4f}")
        ...
        plt.tight_layout()
        results_path = os.path.join(figures_dir, f'fruit_vision_multi_run_results_{args.dataset}_{freeze_status}.png')
        ...
        # 保存最终结果到文件
        results_summary_path = os.path.join(result_folder, f'fruit_vision_results_summary_{args.dataset}_{freeze_status}.txt')
        with open(results_summary_path, 'w', encoding='utf-8') as f:
            f.write(f"{'='*80}\n")
            f.write(f"FruitVision模型 - {args.dataset.upper()}数据集 - 多次运行结果摘要\n")
            f.write(f"{'='*80}\n\n")
            f.write(f"实验配置:\n")
            f.write(f"  数据集: {args.dataset}\n")
            f.write(f"  骨干网络: {freeze_status}\n")
